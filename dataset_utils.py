...
def get_train_dataset(bsize = 32):
    ...
    print(f"The length of train dataset is: {train_size}")
    print(f"The length of validation dataset is: {val_size}")

    train_dataloader = DataLoader(train_subset, batch_size=bsize, shuffle=False, collate_fn= custom_collate_fn)
    validation_dataloader = DataLoader(val_subset, batch_size=bsize, shuffle=False, collate_fn= custom_collate_fn)
...
def get_dataset(bsize = 32):
    ...
    # print(f"The length of validation dataset is: {len(val_dataset)}")
    # print(f"The length of test dataset is: {len(test_dataset)}")

    # train_dataloader = DataLoader(train_dataset, batch_size=bsize, shuffle=True, collate_fn= custom_collate_fn)
    # validation_dataloader = DataLoader(val_dataset, batch_size=bsize, shuffle=False, collate_fn= custom_collate_fn)
    # test_dataloader = DataLoader(test_dataset, batch_size=bsize, shuffle=False, collate_fn= custom_collate_fn)
    ...
    train_size = int(0.9 * len(train_dataset))
    val_size = len(train_dataset) - train_size
    train_dataset, val_subset = random_split(train_dataset, [train_size, val_size])

    test_subset = load_dataset("squad", split="validation")
    test_subset= test_subset.map(tokenize_and_preprocess)
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=bsize, shuffle=True, collate_fn= custom_collate_fn)
    validation_dataloader = DataLoader(val_subset, batch_size=bsize, shuffle=False, collate_fn= custom_collate_fn)
    test_dataloader = DataLoader(test_subset, batch_size=bsize, shuffle=False, collate_fn= custom_collate_fn)
